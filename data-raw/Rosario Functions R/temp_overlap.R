...
temp_overlap_test <- function(data, method, nsim) {
  ...
  int_no <- ncol(data)
  obs_overlap <- as.data.frame(combinations(
  ...
  for (i in 1:nrow(obs_overlap)) {
    ...
    if (method == "pianka") {
      obs_overlap[which(obs_overlap[, 1] == ind1 & obs_overlap[, 2] == ind2), 3] <- pianka_index(
        data[which(rownames(data) == ind1), ],
  ...
  for (k in 1:nsim) {
    ...
    null_overlap <- as.data.frame(combinations(
      n = length(rownames(pseudo_assem)),
